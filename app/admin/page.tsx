...
import { SuperAdminOnly, useAcademyAccess } from "@/app/components/AcademyAccess";
...
function AdminContent() {
  const { academyId } = useAcademyAccess();
  ...
  const toggleAcademy = async (academy: Academy) => {
    if (academy.active && !window.confirm(`${academy.name} 이용을 중지할까요? 해당 학원의 원장과 강사는 즉시 접근할 수 없습니다.`)) return;
    setError("");
    const { error: toggleError } = await supabase.rpc("super_admin_set_academy_active", { target_academy_id: academy.id, enabled: !academy.active });
  ...
  return <main className="min-h-screen flex-1 bg-slate-950 px-4 py-8 text-white"><div className="mx-auto max-w-7xl"><header><p className="text-sm font-bold tracking-[.22em] text-cyan-400">PLATFORM CONTROL</p><h1 className="mt-2 text-3xl font-black">전체 학원 최고관리자</h1><p className="mt-2 text-sm text-slate-400">등록된 학원과 원장·강사 계정을 한곳에서 확인합니다.</p></header>
    ...
    {loading ? <div className="mt-6 rounded-2xl bg-slate-900 p-12 text-center text-slate-400">전체 학원을 불러오는 중...</div> : <>
      ...
      <section className="mt-6 grid gap-4">{academies.map((academy) => { const academyMembers=members.filter((m) => m.academy_id===academy.id); const owners=academyMembers.filter((m) => m.role==="owner"||m.role==="super_admin"); const teachers=academyMembers.filter((m) => m.role==="teacher"); const protectedAcademy=academy.id===academyId; return <article key={academy.id} className="rounded-2xl bg-slate-900 p-6 ring-1 ring-white/10"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><h2 className="text-xl font-black">{academy.name}</h2><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${academy.active?"bg-emerald-400/15 text-emerald-300":"bg-slate-700 text-slate-400"}`}>{academy.active?"운영 중":"이용 중지"}</span></div><p className="mt-1 text-sm text-slate-400">학원 코드 {academy.code}</p></div><div className="flex items-start gap-4"><div className="text-right text-sm text-slate-300"><p>원장 {owners.length}명 · 강사 {teachers.filter((m)=>m.active).length}명</p><p className="mt-1 text-amber-300">승인 대기 {teachers.filter((m)=>m.approval_status==="pending").length}명</p></div><button disabled={protectedAcademy} onClick={()=>void toggleAcademy(academy)} className={`rounded-lg px-3 py-2 text-sm font-bold disabled:cursor-not-allowed disabled:bg-slate-800 disabled:text-slate-500 ${academy.active?"bg-red-500/15 text-red-300":"bg-emerald-500 text-white"}`}>{protectedAcademy?"최고관리자 학원":academy.active?"이용 중지":"이용 복구"}</button></div></div><div className="mt-5 grid gap-2 sm:grid-cols-2">{owners.map((owner)=><div key={owner.email||owner.display_name} className="rounded-xl bg-slate-800 px-4 py-3"><b>{owner.display_name}</b><p className="text-xs text-slate-400">{owner.email} · {owner.role==="super_admin"?"최고관리자":"원장"}</p></div>)}</div></article>; })}</section>
    </>}
  </div></main>;
}
