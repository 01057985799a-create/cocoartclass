...
function FinancePageContent() {
  ...
  const load = useCallback(async () => {
    ...
    const results = await Promise.all([
      supabase.from("students").select("id,name,active,created_at,updated_at").order("name"),
  ...
  const withdrawnStudents = students.filter((student) => !student.active);
  const feeRows = activeStudents.map((student) => ({ student, enrollment: enrollments.find((e) => e.student_id === student.id), payment: payments.find((p) => p.student_id === student.id && p.billing_month === monthDate) })).filter((row) => row.enrollment);
  ...
  const studentYearRows = useMemo(() => Array.from({ length: 12 }, (_, index) => {
    const monthNumber = index + 1;
    const monthStart = new Date(year, index, 1);
    const monthEnd = new Date(year, index + 1, 0, 23, 59, 59, 999);
    const enrolled = students.filter((student) => {
      const joinedAt = new Date(student.created_at);
      const leftAt = student.active ? null : new Date(student.updated_at);
      return joinedAt <= monthEnd && (!leftAt || leftAt > monthEnd);
    }).length;
    const joined = students.filter((student) => { const date = new Date(student.created_at); return date >= monthStart && date <= monthEnd; }).length;
    const withdrawn = students.filter((student) => { if (student.active) return false; const date = new Date(student.updated_at); return date >= monthStart && date <= monthEnd; }).length;
    return { month: monthNumber, enrolled, joined, withdrawn };
  }), [students, year]);
  ...
  return <main className="min-h-screen flex-1 bg-[#f8f4ed] px-4 py-8"><div className="mx-auto max-w-7xl">
    ...
    {loading ? <div className="mt-5 rounded-2xl bg-white p-12 text-center">자료를 불러오는 중...</div> : <>
      <section className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-6">{[["이번 달 수입", won(income)], ["이번 달 지출", won(expense)], ["순이익", won(income - expense)], ["미납금", won(overdueTotal)], ["현재 재원생", `${activeStudents.length}명`], ["퇴원생", `${withdrawnStudents.length}명`]].map(([label, value], i) => <article key={label} className={`rounded-2xl border border-[#e7ddd3] p-5 ${i === 2 ? "bg-emerald-800 text-white" : "bg-white"}`}><p className={`text-sm ${i === 2 ? "text-emerald-100" : "text-zinc-500"}`}>{label}</p><p className={`mt-3 text-2xl font-black ${i === 3 ? "text-red-600" : ""}`}>{value}</p>{i === 3 && <p className="mt-2 text-xs text-zinc-500">{overdue.length}명 미납</p>}</article>)}</section>
      ...
      {tab === "year" && <div className="mt-5 grid gap-5"><section className="rounded-2xl border border-[#e7ddd3] bg-white p-5"><h2 className="text-xl font-black">{year}년 재무 현황</h2><div className="mt-5 grid gap-3">{yearRows.map((row) => <div key={row.month} className="grid grid-cols-[44px_1fr_auto] items-center gap-3"><b>{row.month}월</b><div className="h-3 overflow-hidden rounded-full bg-zinc-100"><div className="h-full bg-emerald-600" style={{ width: `${Math.min(100, row.income ? 20 + row.income / Math.max(1, ...yearRows.map((r) => r.income)) * 80 : 0)}%` }} /></div><span className="min-w-28 text-right font-bold">{won(row.net)}</span></div>)}</div></section>
        <section className="rounded-2xl border border-[#e7ddd3] bg-white p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-xl font-black">{year}년 재원·퇴원 현황</h2><p className="mt-1 text-sm text-zinc-500">각 월 말 재원생과 해당 월 신규·퇴원 인원을 보여줍니다.</p></div><div className="flex gap-4 text-xs font-bold"><span className="text-emerald-700">● 재원생</span><span className="text-blue-600">＋ 신규</span><span className="text-rose-600">－ 퇴원</span></div></div><div className="mt-6 overflow-x-auto"><div className="grid min-w-[760px] grid-cols-12 gap-2 border-b border-zinc-200 pb-3">{studentYearRows.map((row) => { const max = Math.max(1, ...studentYearRows.map((item) => item.enrolled)); return <article key={row.month} className="flex flex-col items-center justify-end text-center"><div className="mb-2 flex h-36 w-full items-end justify-center"><div className="w-9 rounded-t-lg bg-emerald-600 transition-all" style={{ height: `${Math.max(row.enrolled ? 12 : 0, row.enrolled / max * 100)}%` }} /></div><b className="text-sm">{row.month}월</b><strong className="mt-1 text-emerald-800">{row.enrolled}명</strong><div className="mt-1 flex gap-1 text-[10px]"><span className="text-blue-600">+{row.joined}</span><span className="text-rose-600">-{row.withdrawn}</span></div></article>; })}</div></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[620px] text-sm"><thead className="bg-zinc-50 text-zinc-500"><tr><th className="p-3 text-left">구분</th>{studentYearRows.map((row) => <th key={row.month} className="p-3 text-center">{row.month}월</th>)}</tr></thead><tbody><tr className="border-t"><th className="p-3 text-left text-emerald-800">재원생</th>{studentYearRows.map((row) => <td key={row.month} className="p-3 text-center font-bold">{row.enrolled}</td>)}</tr><tr className="border-t"><th className="p-3 text-left text-blue-600">신규</th>{studentYearRows.map((row) => <td key={row.month} className="p-3 text-center">{row.joined}</td>)}</tr><tr className="border-t"><th className="p-3 text-left text-rose-600">퇴원</th>{studentYearRows.map((row) => <td key={row.month} className="p-3 text-center">{row.withdrawn}</td>)}</tr></tbody></table></div><p className="mt-4 rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-800">퇴원 월은 학생 상태가 ‘퇴원’으로 변경된 마지막 수정일을 기준으로 계산합니다.</p></section></div>}
